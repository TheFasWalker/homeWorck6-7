...
app.post('/api/cart', (req, res) => {
    ...
    fs.readFile('./server/db/basket.json', 'utf-8', (err, data) => {
        if (err) res.send(JSON.stringify({ result: 0, err }));
            ...
            cart.push(req.body);
            fs.writeFile('./server/db/basket.json', JSON.stringify(cart,null,'  '), (err) => {
...
app.put('/api/cart/:id', (req, res) => {
    ...
    fs.readFile('./server/db/basket.json', 'utf-8', (err, data) => {
        if (err) res.send(JSON.stringify({ result: 0, err }));
            ...
            fs.writeFile('./server/db/basket.json', JSON.stringify(cart,null,'  '), (err) => {
                if (err) res.end(JSON.stringify({ result: 0, err }));
                else res.end(JSON.stringify({ result: 1 },null,'  '));
            });
        }
    });
});
app.delete('/api/cart/:id', (req, res)=>{
    fs.readFile('./server/db/basket.json', 'utf-8', (err, data) => {
        if(err) res.send(JSON.stringify({result:0, err}));
            ...
            if(cart[basketItem].count > 1){             // проверяем количество
                cart[basketItem].count -=1              // если количество >1 то уменьшаем количество на 1
            }else{
                // delete cart[basketIds]
                 cart.splice(basketItem, 1);
            }
...
app.put('/api/statisks',(req,res)=>{
    console.log('обработка клика')
    fs.readFile('./server/db/clicked.json', 'utf-8', (err, data) => {
        if (err) res.send(JSON.stringify({ result: 0, err}));
        else{
            const statisks = JSON.parse(data);
            statisks.push(req.body);
            fs.writeFile('./server/db/clicked.json', JSON.stringify(statisks ,null,'  '), (err) => {
                if (err) res.end(JSON.stringify({ result: 0, err }));
                else res.end(JSON.stringify({ result: 1 },null,''));
            });
        }
